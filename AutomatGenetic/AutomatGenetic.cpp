    ...
    void mutate() {
        ...
        for (int i = 0; i < mutationSteps; i++)
        {
            if (rand() % 2 < 1 / mutationSteps) {
                ...
                delta = 0;
                for (int i = 0; i < mutationSteps+1; i++)
                {
                    delta += 1 / pow(2, i);
                    if (1)
                    {
...
public:
    ...
    std::vector<Individ> crossover(Individ parent1, Individ parent2)
    {
        ...
        std::uniform_real_distribution<double> alpha(0.01, 1);
    ...
    /*Individ randomSubChoice(Individ start, Individ end, std::mt19937& g) {
        std::uniform_int_distribution<>  dis(0, std::distance(start, end) - 1);
    }

    Individ randomChoice(Individ start, Individ end) {
        static std::random_device rd;
        static std::mt19937 gen(rd());
        return 
    }*/

    double compFunct(Individ ind1, Individ ind2) {
